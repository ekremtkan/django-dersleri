...
class LoginForm(forms.Form):
    ...
    def clean(self):
        ...
        if username and password:
            ...
            if not user:
                raise forms.ValidationError('Kullanıcı Adi ve Parolayı yanlış girdiniz')
...
class RegisterForm(forms.ModelForm):
    ...
    first_name = forms.CharField(max_length=100, label='Adı')
    last_name = forms.CharField(max_length=100, label='Soyadı')
    class Meta:
        model = User
        fields = [
            'username',
...
class DeleteForm(RegisterForm):
    username = forms.CharField(max_length=100, label='Kullanıcı Adı')
    password1 = forms.CharField(max_length=100, label='Parola', widget=forms.PasswordInput)
    delete_time = forms.DateTimeField()
    class Meta:
        model = User
        fields = [
            'username',
